...
connection_string = "mongodb+srv://admin:CpQXiwP1YfHbBASR@users.zaa3zwk.mongodb.net/?retryWrites=true&w=majority&appName=users"
...
def create_chat(chat_name, auth):
    ...
    users.update_one({"auth": auth}, {"$push": {"chats": [chat_name]}})
...
def upload_message(message, chat_name, username):
    ...
    if username in chats.find_one({"name": chat_name})["users"]:
        message = f"username:{username}-message:{message}"
        chats.update_one({"name": chat_name}, {"$push": {"messages": message}})
    else:
...
def add_user_to_chat(chat_name, new_user, username):
    ...
    users.update_one({"username": new_user}, {"$push": {"chats": [chat_name]}})
